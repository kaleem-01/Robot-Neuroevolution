...
from evolve_body import build_robot_from_body
from evolve_graph import _repair_graph
import mujoco as mj
...
def build_graph_from_body(body: BodyGenome):
...
def build_robot_from_graph(graph: DiGraph):
...
def quick_motion_screen(body: BodyGenome, spawn: List[float]) -> bool:
    ...
    try:
        graph = build_graph_from_body(body)
        robot = build_robot_from_graph(graph)
    except Exception:
        return False
...
def build_world_and_model(graph, spawn):
    # Always make a fresh robot/spec per rollout
    mj.set_mjcb_control(None)
    robot = construct_mjspec_from_graph(graph)   # fresh, unattached
    world = OlympicArena()
    world.spawn(robot.spec, spawn_position=spawn)
    model = world.spec.compile()                 # only compile root
    data  = mj.MjData(model)
...
def experiment(
    ...
    world, model, data = build_world_and_model(graph, spawn_pos)
...
def init_population_from_genotypes(pop_size: int) -> Tuple[List[GraphBody], List[BodyGenome]]:
    ...
    while len(graphs) < pop_size and attempts < pop_size * 20:
        attempts += 1
        genome = sample_body_genome(RNG)
        if not quick_motion_screen(genome, SPAWN_START):
            continue
        try:
            graph = build_graph_from_body(genome)
            core = build_robot_from_graph(graph)
            graphs.append(GraphBody(core=core, graph=graph, fitness=-np.inf, progress_x=0.0))
...
def rollout_and_score_graph(graph: "DiGraph", duration: float, spawn: List[float]) -> Tuple[float, float]:
    ...
    world, model, data = build_world_and_model(graph, spawn)

    cpg_cb = cpg_controller_factory(model)
    ctrl = Controller(controller_callback_function=lambda m, d: cpg_cb(m, d, body=robot), tracker=tracker)
    
    hist, _sim_t = experiment(graph, controller=ctrl, duration=duration, mode="simple", spawn_pos=spawn)
    fit = fitness_function(hist)
    px  = last_x(hist)
    return fit, px

def _worker_eval_graph(args):
    graph, duration, spawn = args
    return rollout_and_score_graph(graph, duration=duration, spawn=spawn)

def evaluate_population_graph(pop_graphs: List["DiGraph"], duration: float, spawn: List[float]) -> List[Tuple[float, float]]:
    results = []
    for G in pop_graphs:
        results.append(_worker_eval_graph((G, duration, spawn)))
    return results
...
def evolve() -> Tuple[BodyGenome, List[float], List[float]]:
    ...
    while cur_gen < GENERATIONS:
        ...
        # Evaluate (graphs)
        results = evaluate_population_graph([gb.graph for gb in pop_graphs], duration=duration, spawn=choose_spawn(cur_gen, RNG))
